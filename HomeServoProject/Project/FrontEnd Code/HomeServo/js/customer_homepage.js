...
if(customerId){
    ...
    .catch(error =>{
        window.alert("error fetching of customer details")
    ...
    for( let i=0;i<data.data.works.length;i++){
      ...
      <td class="contentcolor">
       ...
       ${data.data.works[i].address.state},
        ...
        <td><div > <strong>${totalamount}</strong></div></td>
        <td>
        <div > 
        <strong>
        ...
        <td><div class='paymode'>PAY</div></td>
        <td >
         <div class='success'>Succesfull</div>
         <div class='success1'>Pending</div>
        </td>

        <td>${localStorage.getItem('vendorId')}</td>
      ...
      for(let j=0; j<data.data.works.length; j++){
        ...
        if(data.data.works[j].cost.mode != null){
          // paymode.style.backgroundColor="red";
        ...
        paymode.addEventListener('click',async ()=>{
          let inp1 = document.getElementsByClassName('inp1')[j].value;
          ...
          if(data.data.works[j].cost.mode == null && inp1!=null ){
            try{
              ...
              if(data2.status == 201){
                ...
                alert('payment completed sucessfull to')
        ...
        alert(" List of works is not avilable");    
     }  
  }
  catch(error){
          ...
          if(response.status == 302){
            ...
            if(data.data.length!=0||data.data.address==null){
              for(let i=0;i<=data.data.length+1;i++){
                ...
                div2.innerHTML+=`
                <table border="2px solid" id="table1" cellPadding="3px">      
                <tr>
